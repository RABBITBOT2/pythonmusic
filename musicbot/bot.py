            ...
            if guild.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        '봇이 음성채널에 들어와있지 않습니다. '
                        '%s입장를 사용해주세요.' % self.config.command_prefix)
...
#######################################################################################################################
    ...
    async def cmd_플레이리스트리셋(self, player, channel):
        """
        Usage:
    ...
    async def cmd_도움(self, message, channel, command=None):
        """
        {command_prefix}도움 [명령어]
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', '노래봇의 명령어 목록입니다. `{}도움 [명령어]`로 명령어의 기능을 확인하세요.').format(prefix)
        if not self.is_all:
            desc += self.str.get('cmd-help-all').format(prefix)
    ...
    async def cmd_아이디(self, author, user_mentions):
        """
        {command_prefix}아이디
    ...
    async def cmd_저장(self, player, url=None):
        """
        Usage:
    ...
    async def cmd_플레이(self, message, player, channel, author, permissions, leftover_args, song_url):
        """
        {command_prefix}플레이 [유튜브링크]
        {command_prefix}플레이 [제목]
    ...
    async def cmd_검색(self, message, player, channel, author, permissions, leftover_args):
        """
        {command_prefix}검색 [서비스] [검색개수] [영상이름]

        서비스:
            - youtube (yt) : 유튜브
            - soundcloud (sc) : 사운드클라우드
            
            ✅(검색을 완료하고 검색기록을 지웁니다.)
            🚫(다음 검색내용을 보거나 검색을 종료합니다.)
            🏁(검색을 종료합니다.)
        """
    ...
    async def cmd_현재재생(self, player, channel, guild, message):
        """
        {command_prefix}현재재생
    ...
    async def cmd_입장(self, channel, guild, author, voice_channel):
        """
        {command_prefix}입장
    ...
    async def cmd_멈추기(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError(self.str.get('cmd-pause-none', '음악이 재생되지 않고 있어요.'), expire_in=30)
    ...
    async def cmd_재생(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError(self.str.get('cmd-resume-none', '일시정지된 음악이 없어요.'), expire_in=30)
    ...
    async def cmd_클리어(self, player, author):
        """
        {command_prefix}클리어
    ...
    async def cmd_기록삭제(self, user_mentions, message, author, permissions, channel, player, index=None):
        """
        {command_prefix}기록삭제 [리스트 번호]

        노래 리스트 안에 있는 노래를 삭제합니다.
        """
    ...
    async def cmd_볼륨(self, message, player, new_volume=None):
        """
        {command_prefix}볼륨 [1~100]

        노래봇의 소리를 1~100으로 설정합니다.
        """
    ...
    @owner_only
    async def cmd_옵션(self, player, option, value):
        """
        Usage:
    ...
    async def cmd_리스트(self, channel, player):
        """
        {command_prefix}리스트
    ...
    async def cmd_청소(self, message, channel, guild, author, search_range=50):
        """
        {command_prefix}청소 [숫자]

        메시지를 청소합니다. 최소: 1 기본: 50 최대: 1000
        """
    ...
    async def cmd_아이디리스트(self, guild, author, leftover_args, cat='all'):
        """
        Usage:
    ...
    async def cmd_역할(self, author, user_mentions, channel, guild, message, permissions, target=None):
        """
        Usage:
    ...
    @owner_only
    async def cmd_셋네임(self, leftover_args, name):
        """
        Usage:
    ...
    async def cmd_셋닉(self, guild, channel, leftover_args, nick):
        """
        Usage:
    ...
    @owner_only
    async def cmd_봇프로필변경(self, message, url=None):
        """
        {command_prefix}봇프로필변경 [링크]
    ...
    async def cmd_다시시작(self, channel):
        """
        {command_prefix}다시시작
        
        봇을 다시 시작합니다.
        """
        await self.safe_send_message(channel, "🐰봇을 다시 시작합니다.")
    ...
    async def cmd_봇강퇴(self, val, leftover_args):
        """
        {command_prefix}봇강퇴
        ...
        if t is None:
            ...
            if t is None:
                raise exceptions.CommandError('찾을수없습니다. `{0}`'.format(val))
        await t.leave()
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if self.config.embeds:
                content = self._gen_embed()
                content.add_field(name='오류', value=e.message, inline=False)
